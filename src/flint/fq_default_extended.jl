...
@doc Markdown.doc"""
    absolute_degree(a::FqDefaultFiniteField)
...
function _as_poly(a::fq_default)
  if a.poly !== nothing
    return a.poly::fq_default_poly
  else
    g = parent(a).backwardmap(a)
    a.poly = g
    return g::fq_default_poly
  end
end
...
@doc Markdown.doc"""
    absolute_frobenius(x::fq_default, n = 1)
...
# internal for now
function _absolute_basis(F::FqDefaultFiniteField)
  if is_absolute(F)
    return basis(F)
  else
    res = elem_type(F)[]
...
const FqDefaultFiniteFieldIDFqDefaultPoly = Dict{Tuple{fq_default_poly, Symbol}, FqDefaultFiniteField}()
